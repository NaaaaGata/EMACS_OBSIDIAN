...
`◆ name.md` は現在のノート、`● name.md` はほかのノートです。現在ノートは赤、直接接続されたノートは青、孤立ノートは灰色で表示されます。グラフは右ペインより大きな仮想キャンバスへ描画され、矢印キーでゲームのマップのように移動できます。

## LaTeX

`latex` と `dvipng` が PATH 上にあれば数式を画像表示します。見つからない場合も数式範囲を色付きで表示するフォールバックが働きます。
