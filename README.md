...
中央の文章は編集ペインの幅に合わせて自動的に折り返されます。Emacsのフレームを狭くした場合も、文章が画面外へはみ出すことはありません。この折り返しは表示だけに作用し、Markdownファイルへ改行を追加しません。

自動作成を保存時だけに限定しているため、`[[apple]]` の入力途中に `a.md`、`ap.md` などが量産されたり、日本語入力が重くなったりしません。

既存リンクを1件だけ変更して保存した場合は、リンク先ノートもリネームされます。たとえば`[[1234]]`を`[[12345]]`へ変更すると、`1234.md`は`12345.md`になり、Vault内のほかのノートにある`[[1234]]`もすべて更新されます。エイリアスと見出しリンクは保持されます。複数リンクの同時変更など、対応関係が曖昧な場合は自動リネームしません。
